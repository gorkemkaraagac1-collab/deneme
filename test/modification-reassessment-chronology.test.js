...
describe("Modification + reassessment kronolojik ödeme zinciri", () => {
  ...
  test("reassessment-only sözleşmede güncel başlık tutarı geçmiş ayları ezmez", () => {
    localStorage.clear();
    const tfrs16 = loadTfrs16();
    const contract = {
      id: "CHRONO-REASSESSMENT-ONLY", companyId: "C-1", monthlyPayment: 15000,
      discountRate: 6, startDate: "2025-01-01", endDate: "2030-12-31",
      paymentFrequency: "monthly", paymentTiming: "arrears", currency: "USD",
      functionalCurrency: "TRY", reportingCurrency: "TRY", status: "active",
      reassessments: [
        { id: "REASS-JUN", status: "APPLIED", effectiveDate: "2025-06-01", type: "FIXED_PAYMENT_CHANGE",
          newTerms: { payment: 13500, discountRate: 6, leaseTerm: "2030-12-31" },
          appliedFromTerms: { monthlyPayment: 12000, discountRate: 6, endDate: "2030-12-31" }, createdAt: "2025-05-01T00:00:00.000Z" },
        { id: "REASS-MAR", status: "APPLIED", effectiveDate: "2026-03-01", type: "FIXED_PAYMENT_CHANGE",
          newTerms: { payment: 15000, discountRate: 6, leaseTerm: "2030-12-31" },
          appliedFromTerms: { monthlyPayment: 13500, discountRate: 6, endDate: "2030-12-31" }, createdAt: "2026-02-01T00:00:00.000Z" }
      ], modifications: []
    };
    const source = tfrs16.resolveContractScheduleSource(contract);
    const paymentAt = month => { const [y, m] = month.split("-").map(Number); return source.schedule.find(row => row.date instanceof Date && row.date.getUTCFullYear() === y && row.date.getUTCMonth() + 1 === m)?.payment; };
    expect(paymentAt("2025-01")).toBeCloseTo(12000, 2);
    expect(paymentAt("2025-05")).toBeCloseTo(12000, 2);
    expect(paymentAt("2025-06")).toBeCloseTo(13500, 2);
