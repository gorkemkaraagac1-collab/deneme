...
// P3 DÜZELTMESİ (test regresyonu — kod DEĞİL, bkz. license-security.test.js
// dosyasındaki AYNI notun tekrarı): VIEWER, requireContractWriteRole
// tarafından KENDİ şirketi için bile PUT/DELETE'te 403 ile reddedilir —
// bu yüzden PUT/DELETE testleri yazma yetkisi olan kullanıcılar
// kullanmalı, GET testleri (VIEWER'a izinli) USER_A/USER_B ile kalabilir.
const USER_A_WRITER = { id: "USER-A2", username: "userA2", role: "ACCOUNTANT", companyIds: [COMPANY_A] };
...
describe("license-service.canAddUserToCompany — gerçek plan limitleri", () => {
  ...
  function fakeDb({ license, userCount }) {
    return {
      query: jest.fn((sql) => {
        // P3 DÜZELTMESİ (test regresyonu — kod DEĞİL): P1'de
        // getActiveCompanyLicense/getTreeActiveUserCount artık ÖNCE
        // organization-service.js'in iki recursive CTE'sini çağırıyor
        // (ağaç zinciri + alt ağaç) — bu fakeDb önceden bunları hiç
        // karşılamıyordu, bu yüzden her ikisi de boş dönüp fonksiyonlar
        // erken çıkıyordu (her zaman NO_ACTIVE_LICENSE/0 kullanıcı).
        // Bu testlerdeki tüm şirketler PARENT'SIZ (düz/tek şirket)
        // olduğundan, ağaç = [companyId]'nin kendisi.
        ...
        // DÜZELTME: getTreeActiveUserCount'un GERÇEK SQL'i
        // "COUNT(DISTINCT uc.user_id)" kullanıyor — "COUNT(*)" değil
        // (P1'de DISTINCT/ACTIVE filtresi eklendiğinde SQL metni
        // değişmiş, bu mock'un koşulu güncellenmemişti). Eski koşul
        // hiçbir zaman eşleşmiyordu, yani currentUsers testte HER ZAMAN
        // 0 dönüyordu — "limite ulaşıldı" (LIMIT_REACHED) senaryosu
        // hiçbir zaman gerçekten tetiklenemiyordu.
        if (sql.includes("FROM user_companies")) {
...
describe("POST /api/auth/register — kullanıcı limiti (uçtan uca)", () => {
  ...
  function mockRegisterFlow({ license, currentUserCount }) {
    clientQueryMock.mockImplementation((sql) => {
      ...
      // P3 DÜZELTMESİ (test regresyonu — kod DEĞİL): P1'de
      // getActiveCompanyLicense() artık ÖNCE getCompanyAncestryChain()
      // ile şirketin ağaç zincirini (kendisi + ataları) okuyor
      // (holding/root lisans mirası için) — bu mock bu sorguyu hiç
      // karşılamıyordu, bu yüzden zincir boş dönüyor, fonksiyon erken
      // "chain.length === 0 -> null" ile çıkıyor ve alttaki company_
      // licenses sorgusuna HİÇ ulaşmıyordu (her zaman NO_ACTIVE_LICENSE).
      // COMPANY_A'nın parent'ı yok (düz/tek şirket) — tek satırlık,
      // kendisi = kökü olan bir zincir dönüyoruz.
      if (sql.includes("WITH RECURSIVE ancestry")) {
        return Promise.resolve({
          rows: [{ id: COMPANY_A, parent_company_id: null, status: "ACTIVE", depth: 0 }]
        });
      }
      // DÜZELTME: getTreeActiveUserCount ÖNCE getDescendantCompanyIds()
      // (bu recursive CTE) ile ağacı bulur — bu dal eksikti, treeIds
      // boş dönüyordu ve fonksiyon "treeIds.length===0 -> 0" ile erken
      // çıkıp altındaki user_companies sorgusuna HİÇ ulaşmıyordu (yani
      // currentUsers her zaman 0'dı, limit hiçbir zaman aşılamıyordu).
      if (sql.includes("WITH RECURSIVE tree")) {
        return Promise.resolve({ rows: [{ id: COMPANY_A }] });
      }
      if (sql.includes("FROM companies") && sql.includes("FOR UPDATE")) {
      ...
      // DÜZELTME: bkz. SUITE 1'deki fakeDb'deki aynı not — gerçek SQL
      // "COUNT(DISTINCT uc.user_id)" kullanıyor, "COUNT(*)" değil.
      if (sql.includes("FROM user_companies")) {
