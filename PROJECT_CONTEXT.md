...
14. **[KRİTİK — TEST İLE DOĞRULANDI, AĞUSTOS 2026]** Kira Modifikasyonu ve Reassessment işlemleri (`createModification`/`applyModification`/`cancelModification`/`createReassessment`/`applyReassessment`/`cancelReassessment`) **backend'e (PostgreSQL) HİÇ YAZMIYOR** — yalnızca `saveContracts()` (localStorage) çağırıyorlar. `persistContractToApi()` (backend PUT /api/contracts/:id, `details` JSONB) bu akışların HİÇBİRİNDEN çağrılmıyor; TEK çağrı yeri contract oluşturma/genel düzenleme formu. Kanıt: `test/modification-management.test.js` ve `test/reassessment-management.test.js` içindeki "Backend persistence" blokları (fetch spy ile). **Risk:** `hydrateContractsFromApi()` her sayfa açılışında `contracts` dizisini backend'den gelen veriyle TAMAMEN değiştiriyor (`contracts = mapped`) — bir modification/reassessment APPLIED edilip sözleşmenin `monthlyPayment`/`endDate`/`discountRate` alanları değiştirildikten SONRA sayfa yenilenir veya başka bir cihazdan açılırsa, bu değişiklikler backend'deki eski veriyle EZİLİP SESSİZCE KAYBOLUR. Düzeltme yapılmadı (bu tur yalnızca test+rapor kapsamındaydı) — `createModification`/`applyModification`/`cancelModification`/`createReassessment`/`applyReassessment`/`cancelReassessment` içine `persistContractToApi(contract, true)` çağrısı eklenmesi gerekiyor.
