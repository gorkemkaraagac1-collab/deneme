document.addEventListener("DOMContentLoaded", () => {
  ...
  /*
     UI NAVIGATION SAFETY WIRING
     Bind the existing sidebar navigation immediately when DOM is ready.
     This is intentionally additive and preserves all existing IDs,
     classes, storage keys, data models, and engines.
  */
  try {
    if (typeof v191WireNavigation === "function") {
      v191WireNavigation();
    }
  } catch (error) {
    console.error("V24 early navigation wiring error:", error);
  }
  ...
  */
  ...
  function v191WireNavigation() {
    ...
    if (!navigation || navigation.dataset.v191NavigationWired === "1") return;
    ...
    navigation.addEventListener("click", event => {
      ...
      if (text.includes("Ay Sonu Kapanış")) {
        event.preventDefault();
        event.stopPropagation();
      ...
      if (text.includes("Risk & Kontroller")) {
        event.preventDefault();
        event.stopPropagation();
    ...
    navigation.dataset.v191NavigationWired = "1";

    document.querySelectorAll(".nav-item").forEach(link => {
      const text = (link.textContent || "").replace(/\s+/g, " ").trim();
      if (
        text.includes("Finansal Raporlama") ||
        text.includes("Ay Sonu Kapanış") ||
        text.includes("Risk & Kontroller")
      ) {
