document.addEventListener("DOMContentLoaded", () => {
  ...
  */
  ...
  function createBulkJournalModal() {
    ...
    document
      ...
      ?.addEventListener(
        "click",
        () => {
          const button = document.getElementById("generateBulkJournals");
          if (button) button.disabled = true;
          Promise.resolve(generateBulkJournals())
            .catch(error => {
              hideLoading();
              showError(error, "Toplu fiş oluşturma");
            })
            .finally(() => {
  ...
  async function generateBulkJournals() {
    ...
    showLoading(`Toplu fişler hazırlanıyor... (0/${totalContracts})`, totalContracts ? 0 : 100);
    ...
    updateLoadingProgress(100, `${bulkJournalData.length} toplu fiş hazırlandı.`);
    hideLoading();
  }
