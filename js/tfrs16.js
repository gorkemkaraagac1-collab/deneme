document.addEventListener("DOMContentLoaded", () => {
  ...
  TFRS 16 ACCOUNTING ENGINE V16.10
  ------------------------------------------------------------
  V15
  ...
  ------------------------------------------------------------
  V16.10 (additive — Financial Reporting Engine built on V16.9)
  - Reusable financial reporting data functions and reporting snapshot.
  - Liability / ROU roll-forwards, maturity, P&L, cash flow and exposure reports.
  - Contract expiry / renewal, modification, reassessment, journal, control and audit reports.
  - Monthly / quarterly / annual reporting aggregation and centralized reconciliation.
  - Multi-currency exposure remains separated; no FX conversion is introduced.
  ...
  */
  ...
  function getControlSummaryReport(){
    ...
    report.totals={totalControls:rptSafeContracts().length*(Array.isArray(CONTROL_CONFIG)?CONTROL_CONFIG.length:0),passed:summary.green||0,warnings:summary.yellow||0,failed:summary.red||0,critical:summary.criticalExceptions||0,openExceptions:(summary.criticalExceptions||0)+(summary.highExceptions||0)+(summary.mediumExceptions||0)+(summary.lowExceptions||0),closedExceptions:0};
    report.rows=Array.isArray(summary.snapshots)?summary.snapshots.map(s=>({contractId:s.contractId||null,overallStatus:s.overallStatus||null,summary:s.summary||{}})):[];
    return rptFinalize(report);
  }
  ...
  /* ==========================================================
     ...
     ----------------------------------------------------------
     Additive close-control layer over V16.10. Existing calculation,
     schedule, journal, audit, risk/control, CFO and reporting
     engines remain the single sources of truth.
  ========================================================== */

  const MONTH_END_CLOSE_VERSION = "V17";
  const CLOSE_TOLERANCE = 0.05;
  const CLOSE_STORAGE_KEY = "gk_tfrs16_month_end_close_v1";
  const CLOSE_HISTORY_STORAGE_KEY = "gk_tfrs16_month_end_close_history_v1";
  ...
  const CLOSE_SEVERITY = Object.freeze({ CRITICAL: "CRITICAL", HIGH: "HIGH", MEDIUM: "MEDIUM", LOW: "LOW" });
  const CLOSE_SCORE_WEIGHTS = Object.freeze({ CRITICAL: 30, HIGH: 20, MEDIUM: 10, LOW: 5 });
  const CLOSE_CONTROLS = Object.freeze([
    { id: "CONTRACT_COMPLETENESS", name: "Contract completeness", severity: "CRITICAL", blocking: true, category: "COMPLETENESS" },
    { id: "CONTRACT_VALIDITY", name: "Contract validity", severity: "HIGH", blocking: true, category: "COMPLETENESS" },
    { id: "SCHEDULE_COMPLETENESS", name: "Payment schedule completeness", severity: "CRITICAL", blocking: true, category: "CALCULATION" },
    { id: "CALCULATION_SUCCESS", name: "Calculation success", severity: "CRITICAL", blocking: true, category: "CALCULATION" },
    { id: "ESCALATION_VALIDATION", name: "Escalation validation", severity: "HIGH", blocking: true, category: "CALCULATION" },
    { id: "MODIFICATION_REVIEW", name: "Modification review", severity: "HIGH", blocking: false, category: "EVENTS" },
    { id: "REASSESSMENT_REVIEW", name: "Reassessment review", severity: "HIGH", blocking: false, category: "EVENTS" },
    { id: "JOURNAL_GENERATION", name: "Journal generation", severity: "CRITICAL", blocking: true, category: "JOURNAL" },
    { id: "JOURNAL_BALANCE", name: "Journal balance", severity: "CRITICAL", blocking: true, category: "JOURNAL" },
    { id: "CURRENT_NON_CURRENT", name: "Current / non-current classification", severity: "CRITICAL", blocking: true, category: "CLASSIFICATION" },
    { id: "LIABILITY_RECONCILIATION", name: "Liability reconciliation", severity: "CRITICAL", blocking: true, category: "RECONCILIATION" },
    { id: "ROU_RECONCILIATION", name: "ROU reconciliation", severity: "CRITICAL", blocking: true, category: "RECONCILIATION" },
    { id: "CASH_PAYMENT_RECONCILIATION", name: "Cash payment reconciliation", severity: "HIGH", blocking: false, category: "RECONCILIATION" },
    { id: "CONTROL_EXCEPTIONS", name: "Control exceptions", severity: "HIGH", blocking: true, category: "CONTROL" },
    { id: "AUDIT_TRAIL_COMPLETENESS", name: "Audit trail completeness", severity: "MEDIUM", blocking: false, category: "AUDIT" },
    { id: "REPORTING_COMPLETENESS", name: "Reporting completeness", severity: "HIGH", blocking: true, category: "REPORTING" }
  ]);

  function closeNumber(value, fallback = 0) {
    const n = Number(value);
    return Number.isFinite(n) ? n : fallback;
  }

  function closeRound(value, digits = 2) {
    const n = closeNumber(value);
    const f = Math.pow(10, digits);
    return Math.round((n + Number.EPSILON) * f) / f;
  }

  function closePeriodOf(date) {
    const d = rptResolveDate(date);
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
  }

  function closeParseDate(value) {
    return rptDate(value);
  }
  ...
  function closeSaveState(state) {
    try { localStorage.setItem(CLOSE_STORAGE_KEY, JSON.stringify(state || {})); return true; }
    catch (error) { return false; }
  }

  function closeLoadHistory() {
    try {
      const raw = localStorage.getItem(CLOSE_HISTORY_STORAGE_KEY);
      if (!raw) return [];
      const parsed = JSON.parse(raw);
      return Array.isArray(parsed) ? parsed : [];
    } catch (error) { return []; }
  ...
  function closeSaveHistory(history) {
    try { localStorage.setItem(CLOSE_HISTORY_STORAGE_KEY, JSON.stringify(history || [])); return true; }
    catch (error) { return false; }
  }

  function closeGetState(period) {
    const state = closeLoadState();
    return state[period] || { period, status: CLOSE_STATUS.NOT_STARTED, locked: false, certified: false, certificationStatus: "PENDING" };
  }

  function closeSetState(period, patch = {}) {
    const state = closeLoadState();
    state[period] = Object.assign(closeGetState(period), patch, { period });
    closeSaveState(state);
    return state[period];
  }

  function closeRecordHistory(snapshot, action = "CLOSE_STATUS") {
    const history = closeLoadHistory();
    const entry = {
      id: `CLOSE-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      timestamp: new Date().toISOString(),
      action,
      period: snapshot?.period || null,
      reportingDate: snapshot?.reportingDate || null,
      status: snapshot?.status || null,
      score: closeNumber(snapshot?.overallScore),
      certified: Boolean(snapshot?.certified),
      exceptionCount: closeNumber(snapshot?.exceptionCount),
      criticalExceptionCount: closeNumber(snapshot?.criticalExceptionCount),
      closedAt: snapshot?.closedAt || null,
      locked: Boolean(snapshot?.locked),
      actor: typeof auditActor === "function" ? auditActor() : "system"
    };
    history.push(entry);
    closeSaveHistory(history.slice(-500));
    return entry;
  }

  function closeAudit(action, snapshot, metadata = {}) {
    try {
      if (typeof recordAuditEvent === "function") {
        return recordAuditEvent({
          action,
          entityType: "MONTH_END_CLOSE",
          entityId: snapshot?.period || null,
          reason: `V17 Month-End Close ${action}`,
          metadata: Object.assign({ version: MONTH_END_CLOSE_VERSION, period: snapshot?.period || null, reportingDate: snapshot?.reportingDate || null }, metadata)
        });
      }
    } catch (error) {}
    return null;
  ...
  function closeActiveContracts() {
    return rptSafeContracts().filter(contract => {
      const status = String(contract?.status || "ACTIVE").toUpperCase();
      return !["EXPIRED", "TERMINATED", "CANCELLED"].includes(status);
    });
  }

  function closeCriticalFields(contract) {
    const missing = [];
    const checks = [
      ["contractId", contract?.id],
      ["company", contract?.company],
      ...
      ["discountRate", contract?.discountRate],
      ["paymentFrequency", contract?.paymentFrequency || contract?.frequency || "MONTHLY"],
  ...
  function closeContractCompletenessCheck(contract) {
    const missing = closeCriticalFields(contract);
    return { valid: missing.length === 0, missing };
  }

  function closeScheduleCheck(contract, reportingDate) {
    const result = { valid: true, missingPeriods: 0, duplicatePeriods: 0, invalidDates: 0, invalidValues: 0, scheduleLength: 0, errors: [] };
    try {
      const built = rptScheduleRows(contract);
      if (built.error) throw new Error(built.error);
      const schedule = Array.isArray(built.schedule) ? built.schedule : [];
      result.scheduleLength = schedule.length;
      if (!schedule.length) {
        result.valid = false;
        result.errors.push("Schedule is empty.");
        return result;
      }
      const seen = new Set();
      schedule.forEach(row => {
        const d = closeParseDate(row?.date);
        if (!d) result.invalidDates += 1;
        else {
          const key = d.toISOString().slice(0, 10);
          if (seen.has(key)) result.duplicatePeriods += 1;
          seen.add(key);
        }
        ["payment", "interest", "principal", "closingLiability"].forEach(key => {
          if (row?.[key] === undefined || row?.[key] === null || !Number.isFinite(Number(row[key]))) result.invalidValues += 1;
        });
      });
      const d = rptResolveDate(reportingDate);
      const end = closeParseDate(contract.endDate);
      const activeAtClose = !end || end >= d;
      if (activeAtClose && schedule.length === 0) result.missingPeriods = 1;
      if (result.duplicatePeriods || result.invalidDates || result.invalidValues || result.missingPeriods) result.valid = false;
      return result;
    } catch (error) {
      result.valid = false;
  ...
  function closeCalculationCheck(contract) {
    try {
      const built = rptScheduleRows(contract);
      const engine = built.engine || (typeof calculateLeaseEngine === "function" ? calculateLeaseEngine(contract) : null);
      const schedule = built.schedule || [];
      const valid = !built.error && Array.isArray(schedule) && schedule.length > 0 && engine && Number.isFinite(Number(engine.liability)) && Number.isFinite(Number(engine.rouAssets));
      return { valid, liability: closeNumber(engine?.liability), rou: closeNumber(engine?.rouAssets), scheduleLength: schedule.length, error: built.error || null };
    } catch (error) { return { valid: false, liability: 0, rou: 0, scheduleLength: 0, error: error?.message || String(error) }; }
  }

  function closeEscalationCheck(contract) {
    try {
      const type = String(contract?.leaseIncreaseType || contract?.escalationType || "").trim();
      if (!type) return { valid: true, applicable: false, errors: [] };
      const rate = contract?.leaseIncreaseRate;
      const amount = contract?.fixedIncrease;
      const errors = [];
      if (type === "fixedRate" && (!Number.isFinite(Number(rate)) || Number(rate) < 0)) errors.push("Invalid fixed-rate escalation.");
      if (type === "fixedAmount" && (!Number.isFinite(Number(amount)) || Number(amount) < 0)) errors.push("Invalid fixed-amount escalation.");
      return { valid: errors.length === 0, applicable: true, errors };
    } catch (error) { return { valid: false, applicable: true, errors: [error?.message || String(error)] }; }
  }

  function closeModificationCheck(contract, reportingDate) {
    const d = rptResolveDate(reportingDate);
    const items = Array.isArray(contract?.modifications) ? contract.modifications : [];
    const pending = items.filter(item => !["APPLIED", "CANCELLED", "REJECTED"].includes(String(item?.status || "DRAFT").toUpperCase()));
    const invalid = pending.filter(item => {
      const effective = closeParseDate(item?.effectiveDate || item?.modificationDate);
      return !effective || !item?.newTerms && item?.revisedLeaseLiability === undefined;
    });
    return { pending: pending.length, invalid: invalid.length, valid: invalid.length === 0, affectedIds: pending.map(x => x.id || null).filter(Boolean), asOf: d.toISOString().slice(0, 10) };
  }

  function closeReassessmentCheck(contract, reportingDate) {
    const d = rptResolveDate(reportingDate);
    const items = Array.isArray(contract?.reassessments) ? contract.reassessments : [];
    const pending = items.filter(item => !["APPLIED", "CANCELLED", "REJECTED"].includes(String(item?.status || "DRAFT").toUpperCase()));
    const invalid = pending.filter(item => !closeParseDate(item?.effectiveDate || item?.reassessmentDate) || item?.revisedLeaseLiability === undefined);
    return { pending: pending.length, invalid: invalid.length, valid: invalid.length === 0, affectedIds: pending.map(x => x.id || null).filter(Boolean), asOf: d.toISOString().slice(0, 10) };
  }

  function closeControlCheck(report) {
    const rows = Array.isArray(report?.rows) ? report.rows : [];
    const open = rows.filter(r => !["RESOLVED", "CLOSED", "WAIVED"].includes(String(r.status || "").toUpperCase()));
    const critical = open.filter(r => String(r.severity || "").toUpperCase() === "CRITICAL");
    const high = open.filter(r => String(r.severity || "").toUpperCase() === "HIGH");
    return { open, critical, high, valid: critical.length === 0 && high.length === 0 };
  }

  function closeAuditCheck(reportingDate) {
    const d = rptResolveDate(reportingDate);
    const report = typeof getAuditTrailReport === "function" ? getAuditTrailReport({ dateTo: rptIsoDate(d) }) : { rows: [] };
    const events = Array.isArray(report?.rows) ? report.rows : [];
    const missing = closeActiveContracts().filter(contract => !events.some(event => event.contractId === contract.id));
    return { valid: true, totalEvents: events.length, contractsWithoutAuditActivity: missing.map(c => c.id), warning: missing.length > 0 };
  }

  function closeBuildCheck(controlId, status, description, severity, affectedContracts = [], resolved = false, extra = {}) {
    return Object.assign({ controlId, controlName: CLOSE_CONTROLS.find(c => c.id === controlId)?.name || controlId, status, severity, description, affectedContracts, resolved }, extra);
  }

  function closeEvaluateChecklist(reportingDate) {
    const d = rptResolveDate(reportingDate);
    const active = closeActiveContracts();
    const checks = [];

    const incomplete = [];
    active.forEach(c => { const r = closeContractCompletenessCheck(c); if (!r.valid) incomplete.push({ id: c.id, missing: r.missing }); });
    checks.push(closeBuildCheck("CONTRACT_COMPLETENESS", incomplete.length ? "FAILED" : "PASSED", incomplete.length ? `${incomplete.length} active contract(s) have missing critical data.` : "All active contracts contain required close fields.", incomplete.length ? "CRITICAL" : "LOW", incomplete.map(x => x.id), incomplete.length === 0, { details: incomplete }));

    const invalidContracts = active.filter(c => { const s = closeContractCompletenessCheck(c); const start = closeParseDate(c.startDate), end = closeParseDate(c.endDate); return !s.valid || !start || !end || end < start || Number(c.monthlyPayment) < 0 || Number(c.discountRate) < 0; });
    checks.push(closeBuildCheck("CONTRACT_VALIDITY", invalidContracts.length ? "FAILED" : "PASSED", invalidContracts.length ? `${invalidContracts.length} active contract(s) failed validity checks.` : "Contract dates and core financial inputs are valid.", invalidContracts.length ? "HIGH" : "LOW", invalidContracts.map(c => c.id), invalidContracts.length === 0));

    const scheduleErrors = [], calcErrors = [], escalationErrors = [];
    active.forEach(c => {
      const schedule = closeScheduleCheck(c, d);
      if (!schedule.valid) scheduleErrors.push({ id: c.id, detail: schedule });
      const calc = closeCalculationCheck(c);
      if (!calc.valid) calcErrors.push({ id: c.id, detail: calc });
      const esc = closeEscalationCheck(c);
      if (!esc.valid) escalationErrors.push({ id: c.id, detail: esc });
    });
    checks.push(closeBuildCheck("SCHEDULE_COMPLETENESS", scheduleErrors.length ? "FAILED" : "PASSED", scheduleErrors.length ? `${scheduleErrors.length} active contract(s) have schedule issues.` : "Schedules are present and structurally valid.", scheduleErrors.length ? "CRITICAL" : "LOW", scheduleErrors.map(x => x.id), scheduleErrors.length === 0, { details: scheduleErrors }));
    checks.push(closeBuildCheck("CALCULATION_SUCCESS", calcErrors.length ? "FAILED" : "PASSED", calcErrors.length ? `${calcErrors.length} active contract(s) failed calculation validation.` : "Liability and ROU calculations are available for active contracts.", calcErrors.length ? "CRITICAL" : "LOW", calcErrors.map(x => x.id), calcErrors.length === 0, { details: calcErrors }));
    checks.push(closeBuildCheck("ESCALATION_VALIDATION", escalationErrors.length ? "FAILED" : "PASSED", escalationErrors.length ? `${escalationErrors.length} contract(s) have escalation issues.` : "Escalation data is valid where applicable.", escalationErrors.length ? "HIGH" : "LOW", escalationErrors.map(x => x.id), escalationErrors.length === 0, { details: escalationErrors }));

    const pendingMods = [], invalidMods = [], pendingReass = [], invalidReass = [];
    active.forEach(c => { const m = closeModificationCheck(c, d), r = closeReassessmentCheck(c, d); if (m.pending) pendingMods.push({ id: c.id, count: m.pending }); if (m.invalid) invalidMods.push({ id: c.id, count: m.invalid }); if (r.pending) pendingReass.push({ id: c.id, count: r.pending }); if (r.invalid) invalidReass.push({ id: c.id, count: r.invalid }); });
    checks.push(closeBuildCheck("MODIFICATION_REVIEW", invalidMods.length ? "FAILED" : (pendingMods.length ? "WARNING" : "PASSED"), invalidMods.length ? "Pending modifications contain missing critical fields." : (pendingMods.length ? `${pendingMods.length} contract(s) have pending modifications requiring review.` : "No pending modification requires review."), invalidMods.length ? "HIGH" : "MEDIUM", (invalidMods.length ? invalidMods : pendingMods).map(x => x.id), invalidMods.length === 0, { details: { pending: pendingMods, invalid: invalidMods } }));
    checks.push(closeBuildCheck("REASSESSMENT_REVIEW", invalidReass.length ? "FAILED" : (pendingReass.length ? "WARNING" : "PASSED"), invalidReass.length ? "Pending reassessments contain missing critical fields." : (pendingReass.length ? `${pendingReass.length} contract(s) have pending reassessments requiring review.` : "No pending reassessment requires review."), invalidReass.length ? "HIGH" : "MEDIUM", (invalidReass.length ? invalidReass : pendingReass).map(x => x.id), invalidReass.length === 0, { details: { pending: pendingReass, invalid: invalidReass } }));

    let journal = { rows: [], totals: {}, reconciliation: { passed: true, difference: 0 } };
    try { journal = getJournalSummaryReport({ endDate: rptIsoDate(d) }); } catch (error) { journal = { rows: [], totals: {}, reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    const journalRows = Array.isArray(journal.rows) ? journal.rows : [];
    const journalErrors = journalRows.filter(r => r.balanced === false || String(r.controlStatus || "").toUpperCase() === "UNBALANCED");
    checks.push(closeBuildCheck("JOURNAL_GENERATION", journal.errors?.length ? "FAILED" : "PASSED", journal.errors?.length ? "Journal summary could not be generated completely." : "Journal summary is available.", journal.errors?.length ? "CRITICAL" : "LOW", [], !journal.errors?.length, { details: journal.errors || [] }));
    checks.push(closeBuildCheck("JOURNAL_BALANCE", journalErrors.length || !journal.reconciliation?.passed ? "FAILED" : "PASSED", journalErrors.length ? `${journalErrors.length} unbalanced journal(s) detected.` : (!journal.reconciliation?.passed ? "Portfolio journal debit / credit reconciliation failed." : "All summarized journals are balanced."), journalErrors.length || !journal.reconciliation?.passed ? "CRITICAL" : "LOW", journalErrors.map(r => r.contractId).filter(Boolean), journalErrors.length === 0 && Boolean(journal.reconciliation?.passed), { details: journalErrors }));

    let cn = {}, liabRoll = {}, rouRoll = {}, maturity = {};
    try { cn = getCurrentNonCurrentReport(d); } catch (error) { cn = { rows: [], totals: {}, reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    try { liabRoll = getLeaseLiabilityRollForwardReport(new Date(d.getFullYear(), d.getMonth(), 1), d); } catch (error) { liabRoll = { rows: [], reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    try { rouRoll = getRuoAssetRollForward(d); } catch (error) { rouRoll = { rows: [], reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    try { maturity = getLeasePaymentMaturityAnalysis(d); } catch (error) { maturity = { rows: [], reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    checks.push(closeBuildCheck("CURRENT_NON_CURRENT", cn.errors?.length || !cn.reconciliation?.passed ? "FAILED" : "PASSED", cn.errors?.length ? "Current / non-current classification failed for one or more contracts." : (!cn.reconciliation?.passed ? "Current + non-current does not reconcile to total liability." : "Current / non-current classification reconciles."), cn.errors?.length || !cn.reconciliation?.passed ? "CRITICAL" : "LOW", (cn.rows || []).filter(r => r.status === "WARNING" || r.status === "ERROR").map(r => r.contractId), !cn.errors?.length && Boolean(cn.reconciliation?.passed), { difference: closeNumber(cn.reconciliation?.difference) }));
    checks.push(closeBuildCheck("LIABILITY_RECONCILIATION", liabRoll.errors?.length || !liabRoll.reconciliation?.passed ? "FAILED" : "PASSED", liabRoll.errors?.length ? "Liability roll-forward contains errors." : (!liabRoll.reconciliation?.passed ? "Liability roll-forward does not reconcile." : "Liability roll-forward reconciles."), liabRoll.errors?.length || !liabRoll.reconciliation?.passed ? "CRITICAL" : "LOW", (liabRoll.rows || []).filter(r => Math.abs(closeNumber(r.reconciliationDifference)) > CLOSE_TOLERANCE).map(r => r.contractId), !liabRoll.errors?.length && Boolean(liabRoll.reconciliation?.passed), { difference: closeNumber(liabRoll.reconciliation?.difference) }));
    checks.push(closeBuildCheck("ROU_RECONCILIATION", rouRoll.errors?.length || !rouRoll.reconciliation?.passed ? "FAILED" : "PASSED", rouRoll.errors?.length ? "ROU roll-forward contains errors." : (!rouRoll.reconciliation?.passed ? "ROU roll-forward does not reconcile." : "ROU roll-forward reconciles."), rouRoll.errors?.length || !rouRoll.reconciliation?.passed ? "CRITICAL" : "LOW", (rouRoll.rows || []).filter(r => Math.abs(closeNumber(r.reconciliationDifference)) > CLOSE_TOLERANCE).map(r => r.contractId), !rouRoll.errors?.length && Boolean(rouRoll.reconciliation?.passed), { difference: closeNumber(rouRoll.reconciliation?.difference) }));
    checks.push(closeBuildCheck("CASH_PAYMENT_RECONCILIATION", maturity.errors?.length || !maturity.reconciliation?.passed ? "WARNING" : "PASSED", maturity.errors?.length ? "Maturity analysis could not be fully validated." : (!maturity.reconciliation?.passed ? "Expected cash payments do not reconcile to principal plus interest." : "Expected cash payments reconcile to principal plus interest."), maturity.errors?.length || !maturity.reconciliation?.passed ? "HIGH" : "LOW", [], !(maturity.errors?.length || !maturity.reconciliation?.passed), { basis: "EXPECTED", actualCashDataAvailable: false }));

    let controlReport = { rows: [], totals: {} };
    try { controlReport = getControlExceptionReport(); } catch (error) { controlReport = { rows: [], totals: {}, errors: [error?.message || String(error)] }; }
    const controls = closeControlCheck(controlReport);
    checks.push(closeBuildCheck("CONTROL_EXCEPTIONS", controls.open.length ? "FAILED" : "PASSED", controls.open.length ? `${controls.open.length} unresolved control exception(s) detected.` : "No unresolved control exceptions detected.", controls.critical.length ? "CRITICAL" : (controls.high.length ? "HIGH" : "LOW"), controls.open.map(r => r.contractId).filter(Boolean), controls.open.length === 0, { details: controls.open }));

    const audit = closeAuditCheck(d);
    checks.push(closeBuildCheck("AUDIT_TRAIL_COMPLETENESS", audit.warning ? "WARNING" : "PASSED", audit.warning ? `${audit.contractsWithoutAuditActivity.length} active contract(s) have no audit activity visible in the close extract.` : "Audit trail activity is present for active contracts.", audit.warning ? "MEDIUM" : "LOW", audit.contractsWithoutAuditActivity, true, { totalEvents: audit.totalEvents }));

    let reporting = null;
    try { reporting = getTfrs16FinancialReportingSnapshot(d); } catch (error) { reporting = { status: "ERROR", dataQuality: { status: "ERROR" }, error: error?.message || String(error) }; }
    const reportingFailed = reporting?.status === "ERROR" || reporting?.dataQuality?.status === "ERROR";
    checks.push(closeBuildCheck("REPORTING_COMPLETENESS", reportingFailed ? "FAILED" : (reporting?.status === "WARNING" ? "WARNING" : "PASSED"), reportingFailed ? "Financial reporting snapshot contains errors." : (reporting?.status === "WARNING" ? "Financial reporting snapshot contains warnings." : "Financial reporting snapshot is ready."), reportingFailed ? "HIGH" : "LOW", [], !reportingFailed, { dataQuality: reporting?.dataQuality || null }));

    return { reportingDate: rptIsoDate(d), period: closePeriodOf(d), checks, supporting: { activeContracts: active.length, journal, currentNonCurrent: cn, liabilityRollForward: liabRoll, rouRollForward: rouRoll, maturity, controls: controlReport, audit, reporting }, generatedAt: new Date().toISOString() };
  }

  function closeScoreFromChecks(checks) {
    let score = 100;
    const deductions = CLOSE_SCORE_WEIGHTS;
    (checks || []).forEach(check => {
      const severity = String(check.severity || "LOW").toUpperCase();
      const weight = closeNumber(deductions[severity], 5);
      if (["FAILED", "ERROR"].includes(String(check.status || "").toUpperCase())) score -= weight;
      else if (String(check.status || "").toUpperCase() === "WARNING") score -= Math.max(1, Math.floor(weight / 2));
    });
    return Math.max(0, Math.min(100, score));
  }

  function closeDetermineStatus(checks, state, score) {
    const blockers = (checks || []).filter(c => (c.status === "FAILED" || c.status === "ERROR") && (c.severity === "CRITICAL" || c.severity === "HIGH") && (CLOSE_CONTROLS.find(x => x.id === c.controlId)?.blocking !== false));
    const warnings = (checks || []).filter(c => c.status === "WARNING" || (c.status === "FAILED" && c.severity === "MEDIUM"));
    if (state?.locked && state?.certified) return CLOSE_STATUS.CLOSED;
    if (blockers.length) return CLOSE_STATUS.BLOCKED;
    if (warnings.length || score < 100) return CLOSE_STATUS.WARNING;
    return CLOSE_STATUS.READY;
  }

  function getMonthEndCloseStatus(reportingDate) {
    const d = rptResolveDate(reportingDate);
    const period = closePeriodOf(d);
    const state = closeGetState(period);
    const evaluated = closeEvaluateChecklist(d);
    const checks = evaluated.checks || [];
    const score = closeScoreFromChecks(checks);
    const status = closeDetermineStatus(checks, state, score);
    const blockers = checks.filter(c => (c.status === "FAILED" || c.status === "ERROR") && (c.severity === "CRITICAL" || c.severity === "HIGH") && (CLOSE_CONTROLS.find(x => x.id === c.controlId)?.blocking !== false));
    const warnings = checks.filter(c => c.status === "WARNING" || (c.status === "FAILED" && c.severity === "MEDIUM"));
    const openExceptions = (evaluated.supporting.controls?.rows || []).filter(r => !["RESOLVED", "CLOSED", "WAIVED"].includes(String(r.status || "").toUpperCase()));
    const criticalExceptions = openExceptions.filter(r => String(r.severity || "").toUpperCase() === "CRITICAL");
    const summary = {
      version: MONTH_END_CLOSE_VERSION,
      reportingDate: rptIsoDate(d), period, status, overallScore: score,
      contracts: { total: rptSafeContracts().length, active: evaluated.supporting.activeContracts, expired: rptSafeContracts().filter(c => String(c.status || "").toUpperCase() === "EXPIRED").length, terminated: rptSafeContracts().filter(c => String(c.status || "").toUpperCase() === "TERMINATED").length },
      calculations: { status: checks.find(c => c.controlId === "CALCULATION_SUCCESS")?.status || "UNKNOWN" },
      schedules: { status: checks.find(c => c.controlId === "SCHEDULE_COMPLETENESS")?.status || "UNKNOWN" },
      journals: { status: checks.find(c => c.controlId === "JOURNAL_BALANCE")?.status || "UNKNOWN", total: evaluated.supporting.journal?.totals?.journalCount || 0, balanced: evaluated.supporting.journal?.totals?.balancedJournals || 0, unbalanced: evaluated.supporting.journal?.totals?.unbalancedJournals || 0 },
      reconciliation: { status: ["LIABILITY_RECONCILIATION", "ROU_RECONCILIATION", "CURRENT_NON_CURRENT"].every(id => ["PASSED"].includes(checks.find(c => c.controlId === id)?.status)) ? "PASSED" : "WARNING", checks: checks.filter(c => ["LIABILITY_RECONCILIATION", "ROU_RECONCILIATION", "CURRENT_NON_CURRENT", "CASH_PAYMENT_RECONCILIATION"].includes(c.controlId)) },
      currentNonCurrent: evaluated.supporting.currentNonCurrent?.totals || {},
      modifications: { pending: checks.find(c => c.controlId === "MODIFICATION_REVIEW")?.details?.pending?.reduce((s, x) => s + closeNumber(x.count), 0) || 0 },
      reassessments: { pending: checks.find(c => c.controlId === "REASSESSMENT_REVIEW")?.details?.pending?.reduce((s, x) => s + closeNumber(x.count), 0) || 0 },
      controls: { open: openExceptions.length, critical: criticalExceptions.length, high: openExceptions.filter(r => String(r.severity || "").toUpperCase() === "HIGH").length },
      auditTrail: evaluated.supporting.audit,
      dataQuality: { status: blockers.length ? "ERROR" : (warnings.length ? "WARNING" : "READY"), errors: blockers.length, warnings: warnings.length },
      ...
      certificationStatus: state.certificationStatus || "PENDING",
  ...
  function getCloseReadiness(reportingDate) {
    const snapshot = getMonthEndCloseStatus(reportingDate);
    return { ready: snapshot.status === CLOSE_STATUS.READY || snapshot.status === CLOSE_STATUS.CLOSED, score: snapshot.overallScore, status: snapshot.status, blockingIssues: snapshot.criticalIssues || [], warnings: snapshot.warnings || [], passedControls: (snapshot.checklist || []).filter(c => c.status === "PASSED").length, totalControls: (snapshot.checklist || []).length, period: snapshot.period, reportingDate: snapshot.reportingDate };
  }

  function getMonthEndCloseSummary(reportingDate) {
    const s = getMonthEndCloseStatus(reportingDate);
    return { period: s.period, reportingDate: s.reportingDate, contractCount: s.contracts.total, activeContractCount: s.contracts.active, calculationStatus: s.calculations.status, journalStatus: s.journals.status, reconciliationStatus: s.reconciliation.status, exceptionCount: s.controls.open, criticalExceptionCount: s.controls.critical, warningCount: s.warnings.length, closeScore: s.overallScore, closeStatus: s.status };
  }

  function getCompanyMonthEndCloseStatus(company, reportingDate) {
    const d = rptResolveDate(reportingDate);
    const companyName = String(company || "");
    const original = rptSafeContracts();
    const subset = original.filter(c => String(c.company || "") === companyName);
    const checks = closeEvaluateChecklist(d);
    const companyIds = new Set(subset.map(c => c.id));
    const filtered = (checks.checks || []).map(c => Object.assign({}, c, { affectedContracts: (c.affectedContracts || []).filter(id => companyIds.has(id)) })).filter(c => c.affectedContracts.length || ["JOURNAL_GENERATION", "JOURNAL_BALANCE", "CURRENT_NON_CURRENT", "LIABILITY_RECONCILIATION", "ROU_RECONCILIATION", "CASH_PAYMENT_RECONCILIATION", "REPORTING_COMPLETENESS"].includes(c.controlId));
    const score = closeScoreFromChecks(filtered);
    const state = closeGetState(checks.period);
    const status = closeDetermineStatus(filtered, state, score);
    let exposure = { rows: [] };
    try { exposure = getCompanyExposureReport(d); } catch (error) {}
    const row = (exposure.rows || []).find(r => String(r.company || "") === companyName) || {};
    return { period: checks.period, reportingDate: rptIsoDate(d), company: companyName, contractCount: subset.length, activeContractCount: subset.filter(c => !["EXPIRED", "TERMINATED", "CANCELLED"].includes(String(c.status || "ACTIVE").toUpperCase())).length, liability: closeNumber(row.totalLiability), rou: closeNumber(row.rou), journalStatus: filtered.find(c => c.controlId === "JOURNAL_BALANCE")?.status || "UNKNOWN", reconciliationStatus: filtered.some(c => c.status === "FAILED" && c.severity === "CRITICAL") ? "FAILED" : "PASSED", exceptions: filtered.filter(c => c.status !== "PASSED"), score, status };
  }

  function getCurrencyMonthEndCloseStatus(currency, reportingDate) {
    const d = rptResolveDate(reportingDate);
    const currencyName = String(currency || "UNSPECIFIED");
    const subset = rptSafeContracts().filter(c => String(c.currency || "UNSPECIFIED") === currencyName);
    const checks = [];
    const completeness = subset.filter(c => !closeContractCompletenessCheck(c).valid);
    const calculations = subset.filter(c => !closeCalculationCheck(c).valid);
    const schedules = subset.filter(c => !closeScheduleCheck(c, d).valid);
    const validity = subset.filter(c => {
      const start = closeParseDate(c.startDate), end = closeParseDate(c.endDate);
      return !start || !end || end < start || Number(c.monthlyPayment) < 0 || Number(c.discountRate) < 0;
    });
    checks.push(closeBuildCheck("CONTRACT_COMPLETENESS", completeness.length ? "FAILED" : "PASSED", completeness.length ? `${completeness.length} contract(s) have missing critical data.` : "Contract completeness passed.", completeness.length ? "CRITICAL" : "LOW", completeness.map(c => c.id), completeness.length === 0));
    checks.push(closeBuildCheck("CONTRACT_VALIDITY", validity.length ? "FAILED" : "PASSED", validity.length ? `${validity.length} contract(s) failed validity.` : "Contract validity passed.", validity.length ? "HIGH" : "LOW", validity.map(c => c.id), validity.length === 0));
    checks.push(closeBuildCheck("SCHEDULE_COMPLETENESS", schedules.length ? "FAILED" : "PASSED", schedules.length ? `${schedules.length} contract(s) have schedule issues.` : "Schedule completeness passed.", schedules.length ? "CRITICAL" : "LOW", schedules.map(c => c.id), schedules.length === 0));
    checks.push(closeBuildCheck("CALCULATION_SUCCESS", calculations.length ? "FAILED" : "PASSED", calculations.length ? `${calculations.length} contract(s) have calculation issues.` : "Calculation completeness passed.", calculations.length ? "CRITICAL" : "LOW", calculations.map(c => c.id), calculations.length === 0));
    let cn = { rows: [], totals: {}, reconciliation: { passed: true, difference: 0 } };
    try { cn = getCurrentNonCurrentReport(d, { currency: currencyName }); } catch (error) { cn = { rows: [], totals: {}, reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    checks.push(closeBuildCheck("CURRENT_NON_CURRENT", cn.errors?.length || !cn.reconciliation?.passed ? "FAILED" : "PASSED", cn.errors?.length ? "Currency classification failed." : (!cn.reconciliation?.passed ? "Currency current / non-current reconciliation failed." : "Currency current / non-current reconciliation passed."), cn.errors?.length || !cn.reconciliation?.passed ? "CRITICAL" : "LOW", (cn.rows || []).filter(r => r.status !== "READY").map(r => r.contractId), !cn.errors?.length && Boolean(cn.reconciliation?.passed)));
    let journal = { rows: [], totals: {}, reconciliation: { passed: true, difference: 0 } };
    try { journal = getJournalSummaryReport({ currency: currencyName, endDate: rptIsoDate(d) }); } catch (error) { journal = { rows: [], totals: {}, reconciliation: { passed: false, difference: 0 }, errors: [error?.message || String(error)] }; }
    checks.push(closeBuildCheck("JOURNAL_BALANCE", journal.errors?.length || !journal.reconciliation?.passed ? "FAILED" : "PASSED", journal.errors?.length ? "Currency journal summary failed." : (!journal.reconciliation?.passed ? "Currency journal debit / credit reconciliation failed." : "Currency journals reconcile."), journal.errors?.length || !journal.reconciliation?.passed ? "CRITICAL" : "LOW", (journal.rows || []).filter(r => !r.balanced).map(r => r.contractId).filter(Boolean), !journal.errors?.length && Boolean(journal.reconciliation?.passed)));
    let control = { rows: [] };
    try { control = getControlExceptionReport(); } catch (error) { control = { rows: [] }; }
    const openControls = (control.rows || []).filter(r => String(r.currency || "UNSPECIFIED") === currencyName && !["RESOLVED", "CLOSED", "WAIVED"].includes(String(r.status || "").toUpperCase()));
    checks.push(closeBuildCheck("CONTROL_EXCEPTIONS", openControls.length ? "FAILED" : "PASSED", openControls.length ? `${openControls.length} unresolved control exception(s) for ${currencyName}.` : "No unresolved control exceptions for this currency.", openControls.some(r => String(r.severity).toUpperCase() === "CRITICAL") ? "CRITICAL" : (openControls.length ? "HIGH" : "LOW"), openControls.map(r => r.contractId).filter(Boolean), openControls.length === 0));
    const score = closeScoreFromChecks(checks);
    const blockers = checks.filter(c => c.status === "FAILED" && ["CRITICAL", "HIGH"].includes(c.severity) && CLOSE_CONTROLS.find(x => x.id === c.controlId)?.blocking !== false);
    const warnings = checks.filter(c => c.status === "WARNING");
    const companies = [...new Set(subset.map(c => String(c.company || "")))].filter(Boolean).map(company => {
      const rows = subset.filter(c => String(c.company || "") === company);
      return { company, contractCount: rows.length, activeContractCount: rows.filter(c => !["EXPIRED", "TERMINATED", "CANCELLED"].includes(String(c.status || "ACTIVE").toUpperCase())).length, status: rows.some(c => calculations.some(x => x.id === c.id)) ? "BLOCKED" : "READY" };
    });
    return { period: closePeriodOf(d), reportingDate: rptIsoDate(d), currency: currencyName, contractCount: subset.length, companyCount: companies.length, score, status: blockers.length ? "BLOCKED" : (warnings.length ? "WARNING" : "READY"), companies, checks, blockers, warnings, note: "FX conversion is not applied; currency remains isolated." };
  }

  function getMonthEndCloseDashboardData(reportingDate) {
    const d = rptResolveDate(reportingDate);
    const close = getMonthEndCloseStatus(d);
    let bs = {}, pl = {}, cf = {}, cfo = {};
    try { bs = getLeaseBalanceSheetImpact(d); } catch (error) {}
    try { pl = getLeaseProfitLossImpact(new Date(d.getFullYear(), d.getMonth(), 1), d); } catch (error) {}
    try { cf = getLeaseCashFlowReport(new Date(d.getFullYear(), d.getMonth(), 1), d); } catch (error) {}
    const companies = [...new Set(rptSafeContracts().map(c => String(c.company || "")))].filter(Boolean).map(company => getCompanyMonthEndCloseStatus(company, d));
    const currencies = [...new Set(rptSafeContracts().map(c => String(c.currency || "UNSPECIFIED")))].map(currency => getCurrencyMonthEndCloseStatus(currency, d));
    try { cfo = getTfrs16CfoSnapshot(d); } catch (error) {}
    return {
      period: close.period, reportingDate: close.reportingDate, status: close.status, score: close.overallScore,
      totalContracts: close.contracts.total, activeContracts: close.contracts.active,
      totalLiability: closeNumber(bs.leaseLiability), currentLiability: closeNumber(bs.currentLiability), nonCurrentLiability: closeNumber(bs.nonCurrentLiability), rouAssets: closeNumber(bs.rouAssets),
      interestExpense: closeNumber(pl.interestExpense), depreciationExpense: closeNumber(pl.depreciationExpense), cashPayments: closeNumber(cf.totalPayments), principal: closeNumber(cf.principal), interest: closeNumber(cf.interest),
      journalCount: close.journals.total, balancedJournalCount: close.journals.balanced, unbalancedJournalCount: close.journals.unbalanced,
      reconciliationStatus: close.reconciliation.status, exceptionCount: close.controls.open, criticalExceptionCount: close.controls.critical,
      renewals90Days: closeNumber(cfo?.renewals?.within90Days), modificationsPending: close.modifications.pending, reassessmentsPending: close.reassessments.pending,
      companyStatus: companies, currencyStatus: currencies, controls: close.checklist, blockers: close.criticalIssues, warnings: close.warnings, cfoSnapshot: cfo,
      dataQuality: close.dataQuality
    };
  }

  function getMonthEndCloseChecklist(reportingDate) {
    const s = getMonthEndCloseStatus(reportingDate);
    return { period: s.period, reportingDate: s.reportingDate, checks: s.checklist, status: s.status, score: s.overallScore };
  }

  function getMonthEndCloseHistory(filters = {}) {
    let rows = closeLoadHistory();
    if (filters.period) rows = rows.filter(r => r.period === filters.period);
    if (filters.status) rows = rows.filter(r => r.status === filters.status);
    if (filters.from) rows = rows.filter(r => String(r.timestamp || "") >= String(filters.from));
    if (filters.to) rows = rows.filter(r => String(r.timestamp || "") <= String(filters.to));
  ...
  function certifyMonthEndClose(reportingDate, input = {}) {
    const d = rptResolveDate(reportingDate), period = closePeriodOf(d);
    const readiness = getCloseReadiness(d);
    if (!readiness.ready && input.force !== true) return { success: false, status: readiness.status, errors: ["Close is not ready for certification."], readiness };
    const actor = String(input.certifiedBy || (typeof auditActor === "function" ? auditActor() : "system"));
    const now = new Date().toISOString();
    const snapshot = getMonthEndCloseStatus(d);
    const state = closeSetState(period, { status: CLOSE_STATUS.CLOSED, locked: input.locked !== false, certified: true, certificationStatus: "CERTIFIED", certifiedAt: now, certifiedBy: actor, comments: String(input.comments || ""), closedAt: now });
    const closedSnapshot = Object.assign({}, snapshot, state, { status: CLOSE_STATUS.CLOSED, certified: true, locked: state.locked, closedAt: now });
    const history = closeRecordHistory(closedSnapshot, "CERTIFIED");
    closeAudit("CLOSE_CERTIFIED", closedSnapshot, { certifiedBy: actor, comments: state.comments, historyId: history.id });
    return { success: true, status: CLOSE_STATUS.CLOSED, period, reportingDate: rptIsoDate(d), certification: state, historyEntry: history };
  }

  function reopenMonthEndClose(reportingDate, input = {}) {
    const d = rptResolveDate(reportingDate), period = closePeriodOf(d), current = closeGetState(period);
    const actor = String(input.reopenedBy || (typeof auditActor === "function" ? auditActor() : "system"));
    const now = new Date().toISOString();
    const state = closeSetState(period, { status: CLOSE_STATUS.REOPENED, locked: false, certified: false, certificationStatus: "REOPENED", reopenedAt: now, reopenedBy: actor, reopenReason: String(input.reason || "") });
    const history = closeRecordHistory(Object.assign({}, current, state, { reportingDate: rptIsoDate(d), status: CLOSE_STATUS.REOPENED }), "REOPENED");
    closeAudit("CLOSE_REOPENED", state, { reopenedBy: actor, reason: state.reopenReason, historyId: history.id });
    return { success: true, period, reportingDate: rptIsoDate(d), status: CLOSE_STATUS.REOPENED, state, historyEntry: history };
  }

  function getCloseCertification(reportingDate) {
    const d = rptResolveDate(reportingDate), period = closePeriodOf(d), state = closeGetState(period);
    return { period, reportingDate: rptIsoDate(d), certified: Boolean(state.certified), certificationStatus: state.certificationStatus || "PENDING", certifiedAt: state.certifiedAt || null, certifiedBy: state.certifiedBy || null, comments: state.comments || "", locked: Boolean(state.locked), closedAt: state.closedAt || null };
  }

  function getTfrs16CloseApprovalReadiness(reportingDate) {
    return getCloseApprovalReadiness(reportingDate);
  }

  function getCloseApprovalReadiness(reportingDate) {
    const r = getCloseReadiness(reportingDate);
    const close = getMonthEndCloseStatus(reportingDate);
    return { ready: r.ready, score: r.score, status: r.status, blockingIssues: r.blockingIssues, warnings: r.warnings, openControls: close.controls.open, reconciliationStatus: close.reconciliation.status, journalStatus: close.journals.status, period: close.period, reportingDate: close.reportingDate };
  }

  function runV17CloseTests() {
    const results = [];
    try {
      const d = new Date();
      const snapshot = getMonthEndCloseStatus(d);
      ...
      const tests = [
        ["ZERO_OR_EXISTING_CONTRACTS", snapshot && snapshot.contracts && Number.isFinite(snapshot.contracts.total)],
        ["CLOSE_CHECKLIST", Array.isArray(snapshot.checklist)],
        ["CLOSE_STATUS", Object.values(CLOSE_STATUS).includes(snapshot.status)],
        ["CLOSE_SCORE", Number.isFinite(snapshot.overallScore) && snapshot.overallScore >= 0 && snapshot.overallScore <= 100],
        ["CLOSE_READINESS", typeof readiness.ready === "boolean"],
        ["JOURNAL_CONTROL", snapshot.journals && Number.isFinite(snapshot.journals.total)],
        ["LIABILITY_RECONCILIATION", Array.isArray(snapshot.checklist) && !!snapshot.checklist.find(c => c.controlId === "LIABILITY_RECONCILIATION")],
        ["ROU_RECONCILIATION", Array.isArray(snapshot.checklist) && !!snapshot.checklist.find(c => c.controlId === "ROU_RECONCILIATION")],
        ["CURRENT_NON_CURRENT", Array.isArray(snapshot.checklist) && !!snapshot.checklist.find(c => c.controlId === "CURRENT_NON_CURRENT")],
        ["CFO_DASHBOARD_DATA", dashboard && dashboard.period === snapshot.period],
        ["COMPANY_CLOSE", [...new Set(rptSafeContracts().map(c => String(c.company || "")))].every(company => !!getCompanyMonthEndCloseStatus(company, d))],
        ["CURRENCY_SEPARATION", Array.isArray(dashboard.currencyStatus)],
        ["CERTIFICATION_MODEL", getCloseCertification(d).period === snapshot.period],
        ["HISTORY_MODEL", Array.isArray(getMonthEndCloseHistory())],
        ["BACKWARD_COMPATIBILITY", Array.isArray(contracts)],
        ["V17_VERSION", MONTH_END_CLOSE_VERSION === "V17"]
      ];
      tests.forEach(t => results.push({ name: t[0], passed: Boolean(t[1]) }));
      return { passed: results.every(r => r.passed), summary: { total: results.length, passed: results.filter(r => r.passed).length, failed: results.filter(r => !r.passed).length }, results };
    } catch (error) {
      return { passed: false, summary: { total: results.length + 1, passed: results.filter(r => r.passed).length, failed: results.filter(r => !r.passed).length + 1 }, results, error: error?.message || String(error) };
    }
  }
  ...
  Object.assign(window.GK_TFRS16, {
    version: "V17",
    V16_10_VERSION: "V16.10",
    MONTH_END_CLOSE_VERSION,
    CLOSE_STATUS,
    CLOSE_SEVERITY,
    CLOSE_CONTROLS,
    CLOSE_SCORE_WEIGHTS,
    ...
    getCloseCertification,
    certifyMonthEndClose,
    reopenMonthEndClose,
    getCloseApprovalReadiness,
    getTfrs16CloseApprovalReadiness,
    runV17CloseTests
  });
