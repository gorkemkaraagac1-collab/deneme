 ...
 *
 * NOT: js/tfrs16-ui.js içinde V26 "Şirket Ekle" modalı da id="company"
 ...
 * (js/tfrs16-ui.js satır ~580), kullanıcının atanmış şirketleri varsa
...
test.describe("smoke — TFRS 16 ana akış", () => {
  ...
  test("kontrat oluştur → detay aç → ödeme planı gör", async ({ stubbedPage, apiStore }) => {
    ...
    // kalma boş/orfan bir statik div ("tfrs16-ui.js tarafından
  ...
  test("dışa aktarma yeni pencerede rapor açar", async ({ stubbedPage }) => {
    ...
    // (js/tfrs16-ui.js satır ~26874). "download" event'i asla ateşlenmez;
